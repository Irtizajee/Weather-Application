...
function Weather() {
    ...
    // let [user_input , setUser_input] = useState("Islamabad")
    let [tem, setTemp] = useState()
    let [windDirection, setWindDirection] = useState()
    let [wind, setWind] = useState()

    let [tem_3TO6, setTemp_3TO6] = useState()
    let [windDirection_3TO6, setWindDirection_3TO6] = useState()
    let [wind_3TO6, setWind_3TO6] = useState()

    let [tem_6TO9, setTemp_6TO9] = useState()
    let [windDirection_6TO9, setWindDirection_6TO9] = useState()
    let [wind_6TO9, setWind_6TO9] = useState()

    let [error, setError] = useState()
    let dataCountryLatLon = {
    ...
    function getWeather() {
        ...
        let arr = country.trim().match(/\S+/g)

        if (country == country.match(/\s+/g)) {
            arr = "Pakistan"
        }
        else if (country == "") {
            arr = "Pakistan"
        ...
        for (let i = 0; i < arr.length; i++) {
            element += arr[i][0].toUpperCase() + arr[i].substring(1).toLowerCase() + " ";
        }
        let Country = element.trim()
        try {
            if (dataCountryLatLon.hasOwnProperty(`${Country}`) === false) {
                setError("Kindly Check Your Country Name")
            }
            else {
                setError("")
                let userReqLon = dataCountryLatLon[Country][1]
                let userReqLat = dataCountryLatLon[Country][0]
                const data = fetch(`https://www.7timer.info/bin/astro.php?lon=${userReqLon}&lat=${userReqLat}&ac=0&unit=metric&output=json&tzshift=0`);
                ...
                data.then(
                    ...
                    (APIdata) => {
                        // console.log(APIdata.dataseries[0].temp2m)
                        setTemp(APIdata.dataseries[0].temp2m)
                        setWindDirection(APIdata.dataseries[0].wind10m.direction)
                        setWind(APIdata.dataseries[0].wind10m.speed)

                        // console.log(APIdata.dataseries[1])
                        setTemp_3TO6(APIdata.dataseries[1].temp2m)
                        setWind_3TO6(APIdata.dataseries[1].wind10m.speed)
                        // console.log(APIdata.dataseries[2])
                        setTemp_6TO9(APIdata.dataseries[2].temp2m)
                        setWind_6TO9(APIdata.dataseries[2].wind10m.speed)

                        // console.log(APIdata.dataseries)
                    }
                )
                    .catch(
                        (error) => {
    ...
    // let [userReq , setUserReq] = useState()



    //     // if  ( user_input === "islamabad"  )
    //     // {
    //     //     setUserReq("Pakistan")
    //     // }
    //     // else{
    ...
    //     // setUserReq("pakistan")
    //     function getWeather(){
    //         if( user_input === "pakistan" || user_input === "karachi" )
    //     {
    //         setUser_input("islamabad")
    //         // setUserReq("pakistan")
    //     }
    //     const weatherAPI = fetch(`https://goweather.herokuapp.com/weather/${user_input}`)
    //     weatherAPI.then(
    //         (resolve)=>{
    //             return   resolve.json()
    //         }
    //     ).then(
    //         (resolveData)=>{
    //             if (!resolveData.temperature)
    //             {
    //                 setTemp("please enter a valid country or city name")
    //                 setTemp()
    //                 setDec()
    //                 setWind()
    //                 return
    ...
    //                 if ( user_input === "islamabad" )
    //                 {
    //                      setUserReq(user_input)

    //                  }
    //                 setTemp(resolveData.temperature)
    //                 setDec(resolveData.description)
    //                 setWind(resolveData.wind)
    //             }
    ...
    // let dateStartForecast_1 = fullDate.getDate() 
    // let dateEndForecast_1 = dateStartForecast_1 + 2
    // console.log(dateStartForecast_1)
    // console.log(dateEndForecast_1);

    // let dateStartForecast_2 = dateEndForecast_1 + 1
    // let dateEndForecast_2 = dateStartForecast_2 + 2
    // console.log(dateStartForecast_2)
    // console.log(dateEndForecast_2)

    // let dateStartForecast_3 = dateEndForecast_2 + 1
    // let dateEndForecast_3 = dateStartForecast_3 + 2
    // console.log(dateStartForecast_3)
    // console.log(dateEndForecast_3)
    ...
    // console.log( "months in string" , months)
        ...
        <>
            <div className='app_body'>
                ...
                <div className='search'>
                    <input onChange={(e) => { setCountry(e.target.value) }} value={country} type="text" className='user_input' />
                    <i onClick={getWeather} style={{ position: "relative", left: "-40px", color: "white", boxSizing: "border-box", padding: "10px", borderRadius: "20px", cursor: "pointer" }} class="fa-solid fa-magnifying-glass"></i>
                </div>
                ...
                <div className='results' > 
                    <div className="weather_update">
                        <div className="city_country_name"> YOUR COUNTRY: {country} </div>
                        <div className="temperature"> Temperature : {tem}°C </div>
                        <div className="description">
                            <p>Wind Direction  :TO {windDirection} </p>
                            Wind: {wind} km/h
                            <br />
                            {months} {date}, {hours}:{minutes}
                        </div>

                    </div>
                    <div className="forscast">
                        <div className='forecast_hadding'>9 Days of Forecasting</div>
                        <div className="forscast_days">
                            <div>3 Days Forecast</div>
                            <div> {tem}°C</div>
                        ...
                        <div className="forscast_days">
                            <div>Next 3 Days &nbsp;&nbsp;</div>
                            <div> {tem_3TO6} °C</div>
                            <div>Wind:  {wind_3TO6}km/h </div>
                        </div>
                        <div className="forscast_days">
                            <div>Next 3 Days &nbsp;&nbsp;</div>
                            <div>{tem_6TO9}°C</div>
                            <div>Wind:  {wind_6TO9}km/h </div>
                        </div>
                    </div>
                </div>
